...
add_task(async function test_processIncoming_applyIncomingBatchSize_smaller() {
  ...
  try {
    ...
    Assert.equal(engine.previousFailed[0], "record-no-0");
    Assert.equal(engine.previousFailed[1], "record-no-8");
...
add_task(async function test_processIncoming_applyIncomingBatchSize_multiple() {
  ...
  engine._store.applyIncomingBatch = async function(records) {
    batchCalls += 1;
    Assert.equal(records.length, APPLY_BATCH_SIZE);
...
add_task(async function test_processIncoming_notify_count() {
  ...
  try {
    ...
    Assert.equal(engine.previousFailed.length, 3);
    Assert.equal(engine.previousFailed[0], "record-no-0");
    Assert.equal(engine.previousFailed[1], "record-no-5");
    Assert.equal(engine.previousFailed[2], "record-no-10");
    ...
    Assert.equal(engine.previousFailed.length, 1);
    Assert.equal(engine.previousFailed[0], "record-no-0");
...
add_task(async function test_processIncoming_previousFailed() {
  ...
  try {
    ...
    Assert.equal(engine.previousFailed.length, 8);
    Assert.equal(engine.previousFailed[0], "record-no-0");
    Assert.equal(engine.previousFailed[1], "record-no-1");
    Assert.equal(engine.previousFailed[2], "record-no-4");
    Assert.equal(engine.previousFailed[3], "record-no-5");
    Assert.equal(engine.previousFailed[4], "record-no-8");
    Assert.equal(engine.previousFailed[5], "record-no-9");
    Assert.equal(engine.previousFailed[6], "record-no-12");
    Assert.equal(engine.previousFailed[7], "record-no-13");
    ...
    Assert.equal(engine.previousFailed[0], "record-no-0");
    Assert.equal(engine.previousFailed[1], "record-no-1");
    Assert.equal(engine.previousFailed[2], "record-no-8");
    Assert.equal(engine.previousFailed[3], "record-no-9");

    // Refetched items that didn't fail the second time are in engine._store.items.
    Assert.equal(engine._store.items["record-no-4"], "Record No. 4");
    Assert.equal(engine._store.items["record-no-5"], "Record No. 5");
...
add_task(async function test_processIncoming_failed_records() {
  ...
  try {
    ...
    for (let i = 0; i < engine.previousFailed.length; i++) {
      Assert.equal(engine.previousFailed[i], BOGUS_RECORDS[i]);
    }
  ...
  try {
    ...
    for (let i = 0; i < 234; i++) {
      ...
      if ((i == 23) || (i == 42) || (i >= 200))
        ...
        Assert.ok(!id in engine._tracker.changedIDs);
    }
