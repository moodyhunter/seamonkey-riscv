...
add_task(async function test_store() {
  ...
  try {
    ...
    Assert.equal(queryres.length, 2);
    Assert.equal(queryres[0].time, TIMESTAMP1);
    Assert.equal(queryres[0].title, "Hol Dir Firefox!");
    Assert.equal(queryres[1].time, TIMESTAMP2);
    Assert.equal(queryres[1].title, "Hol Dir Firefox!");
  } catch (ex) {
...
add_task(async function test_store_create() {
  ...
  try {
    ...
    Assert.equal(queryres[0].time, TIMESTAMP3);
    Assert.equal(queryres[0].title, "The bird is the word!");
  } catch (ex) {
    PlacesTestUtils.clearHistory();
    do_throw(ex);
  }
...
add_task(async function test_remove() {
  ...
  let queryres = queryHistoryVisits(fxuri);
  Assert.equal(queryres.length, 0);
