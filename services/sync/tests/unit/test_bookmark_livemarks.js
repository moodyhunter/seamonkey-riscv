...
add_task(async function test_livemark_invalid() {
  ...
  Assert.equal(-1, (await store.idForGUID(parentGUID)));

  await store.create(lateParentRec);
  let recID = await store.idForGUID(lateParentRec.id, true);
  Assert.ok(recID > 0);
  Assert.equal(PlacesUtils.bookmarks.getFolderIdForItem(recID),
  ...
  Assert.equal(-1, (await store.idForGUID(noFeedURIRec.id, true)));
  ...
  // No exception, but no creation occurs.
  Assert.equal(-1, (await store.idForGUID(lmParentRec.id, true)));
});
