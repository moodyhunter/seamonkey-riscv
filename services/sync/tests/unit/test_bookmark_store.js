...
add_task(async function test_ignore_specials() {
  ...
  record.deleted = true;
  Assert.notEqual(null, (await store.idForGUID("toolbar")));
...
add_task(async function test_bookmark_create() {
  try {
    ...
    Assert.equal((await store.GUIDForId(id)), fxrecord.id);
    Assert.equal(PlacesUtils.bookmarks.getItemType(id),
                 PlacesUtils.bookmarks.TYPE_BOOKMARK);
    Assert.ok(PlacesUtils.bookmarks.getBookmarkURI(id).equals(fxuri));
    Assert.equal(PlacesUtils.bookmarks.getItemTitle(id), fxrecord.title);
    Assert.equal(PlacesUtils.annotations.getItemAnnotation(id, "bookmarkProperties/description"),
                 fxrecord.description);
    Assert.equal(PlacesUtils.bookmarks.getFolderIdForItem(id),
                 PlacesUtils.bookmarks.toolbarFolder);
    Assert.equal(PlacesUtils.bookmarks.getKeywordForBookmark(id), fxrecord.keyword);
    ...
    Assert.equal((await store.GUIDForId(id)), tbrecord.id);
    Assert.equal(PlacesUtils.bookmarks.getItemType(id),
                 PlacesUtils.bookmarks.TYPE_BOOKMARK);
    Assert.ok(PlacesUtils.bookmarks.getBookmarkURI(id).equals(tburi));
    Assert.equal(PlacesUtils.bookmarks.getItemTitle(id), "");
    let error;
    try {
    ...
    Assert.equal(error.result, Cr.NS_ERROR_NOT_AVAILABLE);
    Assert.equal(PlacesUtils.bookmarks.getFolderIdForItem(id),
                 PlacesUtils.bookmarks.toolbarFolder);
    Assert.equal(PlacesUtils.bookmarks.getKeywordForBookmark(id), null);
  } finally {
    _("Clean up.");
    await store.wipe();
  }
...
add_task(async function test_bookmark_update() {
  try {
    ...
    Assert.equal(PlacesUtils.bookmarks.getItemTitle(bmk1_id), "");
    Assert.equal(PlacesUtils.bookmarks.getKeywordForBookmark(bmk1_id), null);
  } finally {
    _("Clean up.");
    await store.wipe();
  }
...
add_task(async function test_folder_create() {
  try {
    ...
    Assert.equal(PlacesUtils.bookmarks.getItemTitle(id), folder.title);
    Assert.equal(PlacesUtils.bookmarks.getFolderIdForItem(id),
                 PlacesUtils.bookmarks.toolbarFolder);
...
add_task(async function test_folder_createRecord() {
  try {
    ...
    Assert.equal(record.children[0], bmk1_guid);
    Assert.equal(record.children[1], bmk2_guid);
...
add_task(async function test_deleted() {
  try {
    ...
    Assert.equal(error.result, Cr.NS_ERROR_ILLEGAL_VALUE);
...
add_task(async function test_move_order() {
  ...
  try {
    ...
    _("Verify order.");
    Assert.equal(PlacesUtils.bookmarks.getItemIndex(bmk1_id), 0);
    Assert.equal(PlacesUtils.bookmarks.getItemIndex(bmk2_id), 1);
    let toolbar = await store.createRecord("toolbar");
    Assert.equal(toolbar.children.length, 2);
    Assert.equal(toolbar.children[0], bmk1_guid);
    Assert.equal(toolbar.children[1], bmk2_guid);
    ...
    _("Verify new order.");
    Assert.equal(PlacesUtils.bookmarks.getItemIndex(bmk2_id), 0);
    Assert.equal(PlacesUtils.bookmarks.getItemIndex(bmk1_id), 1);
...
add_task(async function test_orphan() {
  try {
    ...
    let bmk1_id = PlacesUtils.bookmarks.insertBookmark(
      ...
      PlacesUtils.bookmarks.DEFAULT_INDEX, "Get Firefox!");
    let bmk1_guid = await store.GUIDForId(bmk1_id);
    Assert.equal(PlacesUtils.bookmarks.getFolderIdForItem(bmk1_id),
                 PlacesUtils.bookmarks.toolbarFolder);
    let error;
    try {
    ...
    Assert.equal(PlacesUtils.bookmarks.getFolderIdForItem(bmk1_id),
                 PlacesUtils.bookmarks.toolbarFolder);
    Assert.equal(PlacesUtils.annotations.getItemAnnotation(bmk1_id, PARENT_ANNO),
                 "non-existent");
...
add_task(async function test_reparentOrphans() {
  try {
    ...
    Assert.equal(PlacesUtils.annotations.getItemAnnotation(folder1_id, PARENT_ANNO),
                 folder1_guid);
