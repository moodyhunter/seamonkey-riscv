...
add_task(async function test_processIncoming_error_orderChildren() {
  ...
  try {
    ...
    Assert.equal(new_children.length, 2);
    Assert.equal(new_children[0], folder1_payload.children[0]);
    Assert.equal(new_children[1], folder1_payload.children[1]);
...
async function test_restoreOrImport(aReplace) {
  ...
  try {
    ...
    Assert.ok(bookmarkGuids.has(fxuri.spec));
    if (!aReplace) {
      Assert.ok(bookmarkGuids.has(tburi.spec));
    }
...
add_task(async function test_mismatched_types() {
  ...
  try {
    ...
    _("Old ID: " + oldID);
    Assert.equal(bms.getItemType(oldID), bms.TYPE_FOLDER);
    ...
    Assert.equal(bms.getItemType(newID), bms.TYPE_FOLDER);
    Assert.ok(PlacesUtils.annotations
...
add_task(async function test_misreconciled_root() {
  ...
  Assert.equal((await store.GUIDForId(toolbarIDBefore)), "toolbar");
