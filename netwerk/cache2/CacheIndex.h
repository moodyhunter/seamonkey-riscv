...
public:
  ...
  void WriteToBuf(void *aBuf)
  {
    ...
    memcpy(ptr, mRec->Get()->mHash, sizeof(SHA1Sum::Hash)); ptr += sizeof(SHA1Sum::Hash);
    NetworkEndian::writeUint32(ptr, mRec->Get()->mFrecency); ptr += sizeof(uint32_t);
    NetworkEndian::writeUint64(ptr, mRec->Get()->mOriginAttrsHash); ptr += sizeof(uint64_t);
    NetworkEndian::writeUint32(ptr, mRec->Get()->mExpirationTime); ptr += sizeof(uint32_t);
    NetworkEndian::writeUint16(ptr, mRec->Get()->mOnStartTime); ptr += sizeof(uint16_t);
    NetworkEndian::writeUint16(ptr, mRec->Get()->mOnStopTime); ptr += sizeof(uint16_t);
  ...
  void ReadFromBuf(void *aBuf)
  {
    ...
    mRec->Get()->mOnStartTime = NetworkEndian::readUint16(ptr); ptr += sizeof(uint16_t);
    mRec->Get()->mOnStopTime = NetworkEndian::readUint16(ptr); ptr += sizeof(uint16_t);
  ...
  void Log() const {
    LOG(("CacheIndexEntry::Log() [this=%p, hash=%08x%08x%08x%08x%08x, fresh=%u,"
         ...
         this, LOGSHA1(mRec->Get()->mHash), IsFresh(), IsInitialized(), IsRemoved(),
