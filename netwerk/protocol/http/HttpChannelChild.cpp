...
void
HttpChannelChild::DoOnStartRequest(nsIRequest* aRequest,
...
void HttpChannelChild::DoOnDataAvailable(nsIRequest* aRequest,
  ...
  if (mListener) {
    nsCOMPtr<nsIStreamListener> listener(mListener);
    nsresult rv = 
        listener->OnDataAvailable(aRequest, aContext, aStream, offset, count);
    if (NS_FAILED(rv)) {
...
void HttpChannelChild::ContinueDoNotifyListener() {
  ...
  // requests with this.
  // FIXME LATER Added in Bug 1540759 but Bug 1358060 not in.
