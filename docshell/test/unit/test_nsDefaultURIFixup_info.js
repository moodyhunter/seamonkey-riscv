  ...
  for (let { input: testInput,
    ...
    for (let flags of flagInputs) {
      ...
      if (makeAlternativeURI && alternativeURI != null) {
        ...
        Assert.equal(URIInfo.fixedURI && info.fixedURI.spec, expectedFixedURI, "should get correct fixed URI");
      }
      ...
      if (couldDoKeywordLookup) {
        if (expectKeywordLookup) {
          ...
          Assert.equal(URIInfo.preferredURI.spec, info.fixedURI.spec, "fixed URI should match");
        }
      } else {
        // In these cases, we should never be doing a keyword lookup and
        // the fixed URI should be preferred:
        let prefURI = URIInfo.preferredURI && URUIInfo.preferredURI.spec;
