...
def find_vs_paths():
    ...
    vs_path = os.path.join(pf, 'Microsoft Visual Studio', '2019', 'Community')
    if not os.path.exists(vs_path):
        vs_path = os.path.join(pf, 'Microsoft Visual Studio', '2017',
                               'Community')
        if not os.path.exists(vs_path):
            pf2 = os.environ.get('ProgramFiles')
            vs_path = os.path.join(pf2, 'Microsoft Visual Studio', '2022',
                                   'Community')
            if not os.path.exists(vs_path):
                raise Exception('%s does not exist; VS2017/2019/2022 not installed?' %
                                vs_path)
