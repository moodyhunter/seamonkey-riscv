...
nsTextPaintStyle::InitSelectionColorsAndShadow()
{
  ...
  if (selectionElement &&
      selectionStatus == nsISelectionController::SELECTION_ON) {
    // Fix selection in Bug 509958 after porting Bug 1449010.
    RefPtr<nsStyleContext> sc;
    // Probe for selection and -moz-selection but prefer
    // unprefixed selection after bug 509958.
    sc = mPresContext->StyleSet()->
      ProbePseudoElementStyle(selectionElement,
                              CSSPseudoElementType::selection,
                              mFrame->StyleContext());
    // Probe for -moz-selection pseudo class.
    if (!sc) {
      sc = mPresContext->StyleSet()->
        ProbePseudoElementStyle(selectionElement,
    ...
    // Use found selection pseudo class.
    if (sc) {
      mSelectionBGColor =
