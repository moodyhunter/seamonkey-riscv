...
static bool IsRootPath(const nsAString& aPath) {
  ...
  if (StringEndsWith(aPath, NS_LITERAL_STRING(":\\"))) {
    return true;
  }
  ...
  if (RFindInReadable(NS_LITERAL_STRING("\\"), begin, end)) {
    // Reset iterator:
    aPath.EndReading(end);
    end--;
    ...
    // Volume GUID paths:
    if (StringBeginsWith(lastSegment, NS_LITERAL_STRING("Volume{")) &&
        lastSegment.Last() == L'}') {
      return true;
    }
  }
...
static auto kSpecialNTFSFilesInRoot = {
    NS_LITERAL_STRING("$MFT"),
    NS_LITERAL_STRING("$MFTMirr"),
    NS_LITERAL_STRING("$LogFile"),
    NS_LITERAL_STRING("$Volume"),
    NS_LITERAL_STRING("$AttrDef"),
    NS_LITERAL_STRING("$Bitmap"),
    NS_LITERAL_STRING("$Boot"),
    NS_LITERAL_STRING("$BadClus"),
    NS_LITERAL_STRING("$Secure"),
    NS_LITERAL_STRING("$UpCase"),
    NS_LITERAL_STRING("$Extend")};
...
static bool IsSpecialNTFSPath(const nsAString& aFilePath) {
  ...
  if (FindInReadable(NS_LITERAL_STRING(":$"), iter, end)) {
    return true;
  }
  ...
  iter = begin;
  auto kDelimiters = NS_LITERAL_STRING("\\:");
