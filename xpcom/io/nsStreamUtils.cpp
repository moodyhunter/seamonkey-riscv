...
public:
  NS_DECL_ISUPPORTS_INHERITED
    ...
    , mTarget(aTarget)
  {
    if (aName && strcmp(aName, "~nsInputStreamReadyEvent") == 0) {
      mCreatedFromDestructor = true;
    }
  }

private:
  bool mCreatedFromDestructor = false;
  ~nsInputStreamReadyEvent()
  {
    ...
    if (NS_FAILED(rv) || !val) {
      if (mCreatedFromDestructor) {
         NS_NOTREACHED("Not processing input stream event: Possible recursion");
         return;
      }
