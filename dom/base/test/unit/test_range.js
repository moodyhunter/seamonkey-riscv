...
const UNORDERED_TYPE = Ci.nsIDOMXPathResult.ANY_UNORDERED_NODE_TYPE;
...
function isWhitespace(aNode) {
  return ((/\S/).test(aNode.nodeValue)) ?
         Ci.nsIDOMNodeFilter.FILTER_SKIP :
...
function evalXPathInDocumentFragment(aContextNode, aPath) {
  ...
  var targetType = Ci.nsIDOMNodeFilter.SHOW_ELEMENT;
  var targetNodeName = prefix;
  if (prefix.indexOf("processing-instruction(") == 0) {
    targetType = Ci.nsIDOMNodeFilter.SHOW_PROCESSING_INSTRUCTION;
    targetNodeName = prefix.substring(prefix.indexOf("(") + 2, prefix.indexOf(")") - 1);
  }
  switch (prefix) {
    case "text()":
      targetType = Ci.nsIDOMNodeFilter.SHOW_TEXT |
                   Ci.nsIDOMNodeFilter.SHOW_CDATA_SECTION;
      targetNodeName = null;
      break;
    case "comment()":
      targetType = Ci.nsIDOMNodeFilter.SHOW_COMMENT;
      targetNodeName = null;
      break;
    case "node()":
      targetType = Ci.nsIDOMNodeFilter.SHOW_ALL;
      targetNodeName = null;
  }
...
function processParsedDocument(doc) {
  ...
  Assert.ok(doc instanceof Ci.nsIDOMXPathEvaluator);
...
function do_miscellaneous_tests(doc) {
  ...
  Assert.ok(!baseRange.isPointInRange(startContainer, startOffset));
  Assert.ok(!baseRange.isPointInRange(startContainer, startOffset + 1));
  Assert.ok(!baseRange.isPointInRange(endContainer, endOffset));
  */
