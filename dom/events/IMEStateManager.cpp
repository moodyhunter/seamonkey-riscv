                                       ...
                                       InputContextAction aAction)
{
  ...
  MOZ_LOG(sISMLog, LogLevel::Info,
    ("OnChangeFocusInternal(aPresContext=0x%p (available: %s), "
     ...
     newTabParent.get(), ToString(aAction.mCause).c_str(),
                                ...
                                EditorBase& aEditorBase)
{
  ...
  if (sActiveIMEContentObserver && IsIMEObserverNeeded(aNewIMEState)) {
    ...
    if (!contentObserver->MaybeReinitialize(widget, presContext,
                   ...
                   const InputContextAction& aAction)
{
  MOZ_LOG(sISMLog, LogLevel::Info,
    ("SetInputContextForChildProcess(aTabParent=0x%p, "
     ...
     aTabParent, ToString(aInputContext.mIMEState.mEnabled).c_str(),
                             ...
                             InputContext::Origin aOrigin)
{
  MOZ_LOG(sISMLog, LogLevel::Info,
    ("SetIMEState(aState={ mEnabled=%s, mOpen=%s }, "
     ...
     aContent, TabParent::GetFrom(aContent), aWidget,
     ToString(aAction.mCause).c_str(), ToString(aAction.mFocusChange).c_str(),
     ToChar(aOrigin)));
                                 ...
                                 const InputContextAction& aAction)
{
  MOZ_LOG(sISMLog, LogLevel::Info,
    ("SetInputContext(aWidget=0x%p, aInputContext={ "
     ...
     aWidget,
     ToString(aInputContext.mIMEState.mEnabled).c_str(),
     ToString(aInputContext.mIMEState.mOpen).c_str(),
     NS_ConvertUTF16toUTF8(aInputContext.mHTMLInputType).get(),
     NS_ConvertUTF16toUTF8(aInputContext.mHTMLInputInputmode).get(),
     NS_ConvertUTF16toUTF8(aInputContext.mActionHint).get(),
...
void IMEStateManager::CreateIMEContentObserver(EditorBase& aEditorBase) {
  ...
  OwningNonNull<nsPresContext> presContext(*sPresContext);
  activeIMEContentObserver->Init(widget, presContext, sContent, aEditorBase);
}
