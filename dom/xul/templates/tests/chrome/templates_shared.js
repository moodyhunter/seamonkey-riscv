  ...
  if (queryType == "rdf" && RDF) {
    ...
    if (expectLoggedMessages) {
      Cc["@mozilla.org/consoleservice;1"].
                 getService(Ci.nsIConsoleService).reset();
    }
...
function copyRDFDataSource(root, sourceds)
{
  ...
  while (dsources.hasMoreElements()) {
    sourceds = dsources.getNext().QueryInterface(Ci.nsIRDFDataSource);
  ...
  var newds = Cc["@mozilla.org/rdf/datasource;1?name=in-memory-datasource"].
  ...
  while (sourcelist.hasMoreElements()) {
    ...
    while (props.hasMoreElements()) {
      ...
      if (prop instanceof Ci.nsIRDFResource) {
