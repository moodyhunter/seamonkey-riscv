  ...
  public:
    ...
    bool resolve(ParseNode* const cur, HandleAtom prefixArg = nullptr) {
        ...
        switch (cur->getKind()) {
          ...
          // ParseNodeKind::ComprehensionFor for comprehensions, ParseNodeKind::LexicalScope for
          // legacy comprehensions.  Probably this should be a non-list
          // eventually.
          case ParseNodeKind::ArrayComp:
            MOZ_ASSERT(cur->isArity(PN_LIST));
            MOZ_ASSERT(cur->pn_count == 1);
            MOZ_ASSERT(cur->pn_head->isKind(ParseNodeKind::LexicalScope) ||
                       cur->pn_head->isKind(ParseNodeKind::ComprehensionFor));
