 ...
 *                            expression, pn_kid2 is Yield, ArrayPush,
 ...
 *                          pn_right: body
 * ComprehensionFor;     pn_left: either ForIn or ForOf
 ...
 * YieldStar,
 * Await
 * ArrayComp    list    pn_count: 1
 *                          pn_head: list of 1 element, which is block
 *                          enclosing for loop(s) and optionally
 *                          if-guarded ArrayPush
 * ArrayPush    unary   pn_kid: array comprehension expression
 * Nop          nullary
 */
enum ParseNodeArity
{
  ...
  public:
    ...
    union {
        ...
        struct {                        /* two kids if binary */
            ...
            ParseNode*  right;
            union {
                unsigned iflags;        /* JSITER_* flags for ParseNodeKind::{ComprehensionFor,}For node */
...
#define PNX_NONCONST    0x04            /* initialiser has non-constants */
    ...
    ParseNode* generatorExpr() const {
        MOZ_ASSERT(isKind(ParseNodeKind::GenExp));

        ParseNode* callee = this->pn_head;
        MOZ_ASSERT(callee->isKind(ParseNodeKind::Function));
        ...
        MOZ_ASSERT(body->last()->isKind(ParseNodeKind::LexicalScope) ||
                   body->last()->isKind(ParseNodeKind::ComprehensionFor));
        return body->last();
    }
