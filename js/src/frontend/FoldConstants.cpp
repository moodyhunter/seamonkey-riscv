...
ContainsHoistedDeclaration(JSContext* cx, ParseNode* node, bool* result)
{
  ...
  restart:
    ...
    switch (node->getKind()) {
      ...
      // Legacy array and generator comprehensions use ParseNodeKind::If to represent
      // conditions specified in the comprehension tail: for example,
      // [x for (x in obj) if (x)].  The consequent of such ParseNodeKind::If nodes is
      // either ParseNodeKind::Yield in a ParseNodeKind::Semi statement (generator comprehensions) or
      // ParseNodeKind::ArrayPush (array comprehensions) .  The first case is consistent
      ...
      // banish ParseNodeKind::ArrayPush to the unreachable list, handling it explicitly.
      //
      // We could require that this one weird ParseNodeKind::ArrayPush case be packaged in
      // a ParseNodeKind::Semi, for consistency.  That requires careful bytecode emitter
      ...
      case ParseNodeKind::ComprehensionFor: {
        ...
        if (loopHead->isKind(ParseNodeKind::ForHead)) {
            ...
            MOZ_ASSERT(loopHead->isKind(ParseNodeKind::ForIn) || loopHead->isKind(ParseNodeKind::ForOf));
...
IsEffectless(ParseNode* node)
{
    return node->isKind(ParseNodeKind::True) ||
           ...
           node->isKind(ParseNodeKind::GenExp);
}
        ...
        bool inGenexpLambda)
{
    ...
    if (ParseNode*& catchScope = node->pn_kid2) {
        if (!Fold(cx, &catchScope, parser, inGenexpLambda))
            return false;
    }
