...
var gPrintSetInterface = Ci.nsIPrintSettings;
var doDebug            = false;
...
function displayPropertiesDialog() {
  ...
  try {
    var printingPromptService = Cc["@mozilla.org/embedcomp/printingprompt-service;1"]
...
function loadDialog() {
  ...
  try {
    ...
    printService = Cc["@mozilla.org/gfx/printsettings-service;1"];
    if (printService) {
      printService = printService.getService();
      if (printService) {
        printService = printService.QueryInterface(Ci.nsIPrintSettingsService);
      }
    }
  } catch (e) {}
...
function onLoad() {
  ...
  gWebBrowserPrint = window.arguments[1].QueryInterface(Ci.nsIWebBrowserPrint);
  paramBlock       = window.arguments[2].QueryInterface(Ci.nsIDialogParamBlock);
