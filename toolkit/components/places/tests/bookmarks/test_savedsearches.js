...
add_test(function test_savedsearches_bookmarks() {
  ...
  try {
    ...
    for (let i = 0; i < cc; i++) {
      ...
      Assert.equal(item.itemId, bookmarkId);
...
add_task(async function test_savedsearches_history() {
  ...
  try {
    ...
    for (var i = 0; i < cc; i++) {
      ...
      Assert.equal(node.type, node.RESULT_TYPE_QUERY);
      // test that queries (as containers) have valid itemId
      Assert.equal(node.itemId, searchId);
      node.QueryInterface(Ci.nsINavHistoryContainerResultNode);
      node.containerOpen = true;
    ...
    Assert.equal(tmpFolderNode.itemId, tmpFolderId);
