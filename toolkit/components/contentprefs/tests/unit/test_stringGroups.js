...
function run_test() {
  ...
  function simple_test_methods(aGroup, shouldThrow) {
    ...
    if (shouldThrow) {
      ...
      Assert.equal(cps.setPref(aGroup, prefName, prefValue), undefined);
      Assert.ok(cps.hasPref(aGroup, prefName));
      Assert.equal(cps.getPref(aGroup, prefName), prefValue);
      Assert.equal(cps.removePref(aGroup, prefName), undefined);
      Assert.ok(!cps.hasPref(aGroup, prefName));
    }
  }
  ...
  function complex_test_methods(aGroup) {
    ...
    Assert.ok(cps.hasPref(uri, prefName));
    Assert.ok(cps.hasPref(stringURI, prefName));
    Assert.ok(cps.hasPref(stringObjectURI, prefName));

    Assert.equal(cps.getPref(uri, prefName), prefValue);
    Assert.equal(cps.getPref(stringURI, prefName), prefValue);
    Assert.equal(cps.getPref(stringObjectURI, prefName), prefValue);
  ...
  Assert.equal(cps.setPref(stringObjectURI, "test.5", 5), undefined);
  Assert.equal(cps.setPref(stringURI, "test.2", 2), undefined);
  Assert.equal(cps.setPref(uri, "test.1", 1), undefined);
  ...
  Assert.equal(cps.setPref(uri, "test.4", 4), undefined);
  Assert.equal(cps.setPref(stringObjectURI, "test.0", 0), undefined);
  ...
  Assert.equal(cps.setPref(stringURI, "test.3", 3), undefined);
...
function enumerateAndCheck(prefs, expectedSum, expectedNames) {
  ...
  Assert.equal(expectedNames.length, 0);
}
