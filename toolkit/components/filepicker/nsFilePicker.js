...
function nsFilePicker() {
  ...
  if (lastDirectory) {
    try {
      var dir = Cc[LOCAL_FILE_CONTRACTID].createInstance(nsIFile);
...
nsFilePicker.prototype = {
  ...
  show() {
    ...
    if (this.mParentWindow) {
      ...
      try {
        var appShellService = Cc[APPSHELL_SERV_CONTRACTID].getService(nsIAppShellService);
...
function srGetStrBundle(path) {
  ...
  if (!strBundleService) {
    try {
      strBundleService = Cc[STRBUNDLE_SERV_CONTRACTID].getService(nsIStringBundleService);
    } catch (ex) {
