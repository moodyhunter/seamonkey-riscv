...
const nsIFileView = Ci.nsIFileView;
...
var sfile = Cc[NS_LOCAL_FILE_CONTRACTID].createInstance(nsIFile);
var retvals;
var filePickerMode;
var homeDir;
var treeView;
...
function filepickerLoad() {
  ...
  treeView = Cc[NS_FILEVIEW_CONTRACTID].createInstance(nsIFileView);
...
function setInitialDirectory(directories) {
  ...
  var dirService = Cc[NS_DIRECTORYSERVICE_CONTRACTID]
...
function selectOnOK() {
  ...
  if (allowURLs) {
    try {
      ...
      let fileList = [];
      if (retvals.fileURL instanceof Ci.nsIFileURL)
...
function onDirectoryChanged(target) {
  ...
  var file = Cc[NS_LOCAL_FILE_CONTRACTID].createInstance(nsIFile);
