...
var SearchCallback = {
  searchSucceeded(addons, length, total) {
    ...
    Assert.equal(addons.length, length);
    Assert.equal(length, TESTS[gCurrentTest].length);
    Assert.equal(total, TESTS[gCurrentTest].total);
...
function run_test() {
  ...
  Assert.notEqual(AddonRepository, null);
  gCurrentTest = 0;
  run_current_test();
}
