...
function run_test_1() {
  ...
  AddonRepository.searchAddons("test", 6, {
    searchSucceeded(aAddons) {
      Assert.notEqual(aAddons, null);
      Assert.equal(aAddons.length, 1);
      Assert.equal(aAddons[0].id, "compatmode-normal@tests.mozilla.org");
...
function run_test_2() {
  ...
  AddonRepository.searchAddons("test", 6, {
    searchSucceeded(aAddons) {
      Assert.notEqual(aAddons, null);
      Assert.equal(aAddons.length, 1);
      Assert.equal(aAddons[0].id, "compatmode-strict@tests.mozilla.org");
...
function run_test_3() {
  ...
  AddonRepository.searchAddons("test", 6, {
    searchSucceeded(aAddons) {
      Assert.notEqual(aAddons, null);
      Assert.equal(aAddons.length, 1);
      Assert.equal(aAddons[0].id, "compatmode-ignore@tests.mozilla.org");
